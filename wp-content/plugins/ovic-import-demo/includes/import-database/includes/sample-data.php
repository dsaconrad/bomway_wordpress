...
if ( !class_exists( 'Ovic_Import_Database_Sample_Data' ) ) {
	class Ovic_Import_Database_Sample_Data
	{
		...
		public static function initialize()
		{
			...
			if ( in_array( self::$demo_images_storage, $match ) && false !== get_option( 'import_sample_data_install_sample_package' ) ) {
		...
		public static function download_image( $url, $target )
		{
			...
			if ( !empty( $attachments ) ) {
				...
				foreach ( $attachments as $attachment ) {
					...
					if ( preg_match( '#' . self::$demo_site_pattern . self::$demo_image_pattern . '#i', $attachment_url ) ) {
						...
						$local_file = end( explode( '/wp-content/uploads/', $attachment->guid ) );

						if ( self::download_image( $attachment->guid, $local_file ) ) {
							$done++;
						}
					}
				}
		...
		public static function import_sample_package( $package )
		{
			...
			if ( !is_dir( $path ) ) {
				...
				if ( !is_dir( $path ) ) {
					wp_send_json_error( esc_html__( 'Failed to extract downloaded package to file system.', 'ovic-import' ) );
				}
			} else {
		...
		public static function install_plugin()
		{
			...
			// Verify nonce.
			if ( !isset( $_REQUEST['security'] ) || !wp_verify_nonce( $_REQUEST['security'], 'import_sample_data_ajax_admin' ) ) {
				wp_send_json_error( esc_html__( 'Nonce verification failed. This might due to your working session has been expired. Please reload the page to renew your working session.', 'ovic-import' ) );
			}
			if ( empty( $plugin ) ) {
		...
		public static function download( $uri, $target = '' )
		{
			...
			if ( !empty( $target ) ) {
				...
				// Move file.
				WP_Filesystem_Direct::move( $result, $target, true );
                    ...
                    <div class="popup-content">
                        <div id="sample-data-installation-step-1">
                            <div class="notice-warning settings-error notice">
                                ...
                                <ul>
                                    <li><?php printf( __( 'Installing sample data will replace the content of current website with <a href="%1$s" target="_blank" rel="noopener noreferrer"><strong>%2$s</strong></a>.', 'ovic-import' ), $package['demo'], $package['name'] ); ?></li>
                                    <li><?php esc_html_e( 'You can later uninstall sample data to restore the original data back.', 'ovic-import' ); ?></li>
                                </ul>
                            </div>
							<?php
							if ( array_key_exists( 'sample-page', $package ) && !empty( $package['sample-page'] ) ) :
                                ...
                                <div id="sample-data-installation-options" class="import-sample-data-wrap">
                                    ...
                                    <div class="box-wrap three-col select-page" style="display: none;">
                                        <p><?php esc_html_e( 'Select demo that you want to install.', 'ovic-import' ); ?></p>
                                ...
                                <p style="margin:0;padding:0;">
                                    <label>
                                        <input name="agree" value="1" id="confirm-sample-data-installation"
                                               type="checkbox">
										<?php esc_html_e( 'I understand the impact of installing sample data.', 'ovic-import' ); ?>
                                    </label>
                                </p>
                                <p class="attachment-options" style="padding:0;">
									<?php
									$remote_txt      = esc_html__( 'All demo images will be accessed remotely - FAST IMPORT', 'ovic-import' );
									$local_txt       = esc_html__( 'All demo images will be downloaded and stored locally - SLOW IMPORT', 'ovic-import' );
									$placeholder_txt = esc_html__( 'All demo images will be replaced with gray images from https://via.placeholder.com - FASTEST IMPORT', 'ovic-import' );
									?>
                        ...
                        <div id="sample-data-installation-step-2" class="hidden">
                            <p style="margin-top:0;padding-top:0;">
								<?php esc_html_e( 'There are several stages involved in the process. Please be patient.', 'ovic-import' ); ?>
                            </p>
                            <ul id="install-processes">
                                <li id="install-sample-data-download-package">
                                    <span class="title"><?php esc_html_e( 'Download sample data package.', 'ovic-import' ); ?></span>
                                    <span class="spinner is-active"></span>
                                </li>
                                <li id="install-sample-data-import-data" class="hidden">
                                    <span class="title"><?php esc_html_e( 'Install sample data.', 'ovic-import' ); ?></span>
                                    <span class="spinner is-active"></span>
                                </li>
                                <li id="install-sample-data-attachment" class="hidden">
                                    <span class="title"><?php esc_html_e( 'Download Attachment', 'ovic-import' ); ?></span>
                                    <span class="spinner is-active"></span>
                                </li>
                                <li id="install-sample-data-required-plugins" class="hidden">
                                    <span class="title"><?php esc_html_e( 'Install required plugins.', 'ovic-import' ); ?></span>
                                    <span class="spinner is-active"></span>
                    ...
                    <div class="popup-content">
                        <div id="sample-data-uninstallation-step-1">
                            <div class="notice-warning settings-error notice">
                                ...
                                <ul>
                                    <li><?php esc_html_e( 'Uninstalling sample data will restore the backed up original data before the current sample data was installed..', 'ovic-import' ); ?></li>
                                </ul>
                            </div>
                            <div class="checkbox">
                                <label>
                                    <input name="agree" value="1" id="confirm-sample-data-installation"
                                           type="checkbox">
									<?php esc_html_e( '	I understand the impact of uninstalling sample data.', 'ovic-import' ); ?>
                                </label>
                            </div>
                            <div class="uninstallation-actions">
                                <button data-package="<?php echo esc_attr( $package['id'] ); ?>" id="config-action"
                                        disabled="disabled"
                                        class="button button-primary button-uninstall"><?php esc_html_e( 'Continue', 'ovic-import' ); ?></button>
                                <button id="cancel-action"
                                        class="button button-cancel"><?php esc_html_e( 'Cancel', 'ovic-import' ); ?></button>
                            </div>
                        </div>
                    ...
                    <div class="popup-content">
                        <div id="sample-data-failed_security">
                            <div class="notice-warning settings-error notice" style="margin-bottom:0">
                                <ul>
                                    <li><?php esc_html_e( 'Nonce verification failed. This might due to your working session has been expired. Please reload the page to renew your working session.', 'ovic-import' ); ?></li>
                                </ul>
                            </div>
                        </div>
                    </div>
				...
				case 'failed_package' :
                    ...
                    <div class="popup-content">
                        <div id="sample-data-failed_package">
                            <div class="notice-warning settings-error notice" style="margin-bottom:0">
                                <ul>
                                    <li><?php esc_html_e( 'Failed to get data for the selected sample data package.', 'ovic-import' ); ?></li>
                                </ul>
                            </div>
                        </div>
                    </div>
				...
				case 'missing_package' :
                    ...
                    <div class="popup-content">
                        <div id="sample-data-missing_package">
                            <div class="notice-warning settings-error notice" style="margin-bottom:0">
                                <ul>
                                    <li><?php esc_html_e( 'Missing sample data package to install.', 'ovic-import' ); ?></li>
                                </ul>
                            </div>
                        </div>
                    </div>
		...
		public static function generate_sample_data( $download, $backup_filename )
		{
			...
			if ( !self::prepare_directory( $backup_dir ) ) {
				wp_send_json_error( esc_html__( 'Failed to create directory to store database backup file.', 'ovic-import' ) );
			}
			$exclude_table = array(
				'options',
				'users',
