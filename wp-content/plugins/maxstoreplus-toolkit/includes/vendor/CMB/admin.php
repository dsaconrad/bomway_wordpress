...
class KT_Admin {
	...
	public function __construct() {
	   global $koolshop_toolkit;
        $this->key = $koolshop_toolkit->theme_option;
        $this->metabox_id = sanitize_key(wp_get_theme()->get('Name')).'_kt_option_metabox';
	    $this->plugin_dir = $koolshop_toolkit->getPluginDir();
        $this->plugin_uri = $koolshop_toolkit->getPluginUri();
		// Set our title

		$this->title = __( 'Theme Options', 'kutetheme' );
        if(is_dir($koolshop_toolkit->theme_dir . 'settings/')){
            $config_file = $koolshop_toolkit->theme_dir . 'settings/theme_option.php';
            if ( file_exists($config_file)) {
                $this->config = include( $config_file );
            }else{
                $config_file = $koolshop_toolkit->plugin_dir . 'settings/theme_option.php';
                $this->config = include( $config_file );
            }
        }else{
            $config_file = $koolshop_toolkit->plugin_dir . 'settings/theme_option.php';
    ...
    function kt_admin_styles() {
        wp_enqueue_style(  'add_stylesheet', $this->plugin_uri . 'classes/CMB/css/style.css' );
        wp_enqueue_script( 'kt-theme-option-js', $this->plugin_uri . 'classes/CMB/js/custom.js', array( 'jquery' ), '1.0', true );
    }
    /**
