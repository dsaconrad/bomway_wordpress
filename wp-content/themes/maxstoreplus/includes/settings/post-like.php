...
function sl_enqueue_scripts() {
	wp_enqueue_script( 'simple-likes-public-js', get_theme_file_uri( 'js/simple-likes-public.min.js' ), array( 'jquery' ), '1.0', false );
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'like' => __( 'Like', 'maxstoreplus' ),
		'unlike' => __( 'Unlike', 'maxstoreplus' )
	) ); 
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'maxstoreplus' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'maxstoreplus' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'maxstoreplus' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = __( 'Like', 'maxstoreplus' );
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'maxstoreplus' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'maxstoreplus' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
