...
class Widget_Product_Categories extends WC_Widget
{
	...
	public function __construct()
	{
		...
		$this->widget_description = __( 'A list or dropdown of product categories.', 'maxstoreplus' );
		$this->widget_id          = 'woocommerce_product_categories';
		$this->widget_name        = __( 'Maxstore Plus - Product Categories', 'maxstoreplus' );
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => __( 'Product Categories', 'maxstoreplus' ),
				'label' => __( 'Title', 'maxstoreplus' ),
			),
			'orderby'            => array(
				'type'    => 'select',
				'std'     => 'name',
				'label'   => __( 'Order by', 'maxstoreplus' ),
				'options' => array(
					'order' => __( 'Category Order', 'maxstoreplus' ),
					'name'  => __( 'Name', 'maxstoreplus' ),
				),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Show as dropdown', 'maxstoreplus' ),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => __( 'Show product counts', 'maxstoreplus' ),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => __( 'Show hierarchy', 'maxstoreplus' ),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Only show children of the current category', 'maxstoreplus' ),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Hide empty categories', 'maxstoreplus' ),
			),
		);
	...
	public function widget( $args, $instance )
	{
		...
		if ( $dropdown ) {
			...
			$list_args['show_option_none']           = __( 'No product categories exist.', 'maxstoreplus' );
...
add_action( 'widgets_init', function () { return register_widget( "Widget_Product_Categories" ); } );
