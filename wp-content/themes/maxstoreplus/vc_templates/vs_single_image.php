...
$lazy_load_class = maxstoreplus_get_option( 'kt_enable_lazy', '' ) == 0 ? '' : 'lazy';
...
switch ( $source ) {
	...
	case 'featured_image':
		...
		$image_thumb      = '<img class="vc_single_image-img ' . $lazy_load_class . ' attachment-' . $img_size . '" src="' . $lazy_url . '" data-src="' . $image[0] . '" width="' . $image[1] . '" height="' . $image[2] . '"/>';
		$img              = array(
	...
	case 'external_link':
		...
		$img              = array(
			'thumbnail' => '<img class="vc_single_image-img ' . $lazy_load_class . '" ' . $hwstring . ' src="' . $lazy_url . '" data-src="' . $custom_src . '"/>',
		);
		break;
	default:
		$img = false;
